...
ACCOUNT1='0x799EF7c58CCB496540DAf653BB799C3658F481B2'
ACCOUNT2='0x1111111111111111111111111111111111111111'
...
yellow "  Address of Schnorr contract is NOT sent as constructor argument, as we are using precompiled schnorr verify contract"
any_key
forge create --rpc-url $RPC_URL src/StringInput.sol:StringInput --use $COMPILER_VERSION --unlocked --from $ACCOUNT1 | tee /tmp/transaction-string-input.log
...
any_key
cast send --gas-limit 10000000 --rpc-url $RPC_URL $STRING_INPUT_ADRESS --from $ACCOUNT1 --unlocked 'push(string, bytes32, bytes) (address)' "$STRING" "$PUBLIC_KEY" "$SIGNATURE" | tee /tmp/call-result
